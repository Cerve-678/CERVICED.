...
const FRAME_W = Math.min(SCREEN_WIDTH - 48, 320);
...
export function ServiceImageCropper({
  ...
  const frameH = FRAME_W / frameRatio;
  ...
  const layout = useMemo(() => {
    ...
    const scale = Math.max(FRAME_W / natural.w, frameH / natural.h);
    ...
    try {
      ...
      if (preset.ratio != null && layout && natural) {
        context.crop({
          ...
          width: Math.round(FRAME_W / layout.scale),
  ...
  }, [uri, busy, preset.ratio, layout, natural, frameH, index, uris.length, finish]);
  ...
  return (
    <Modal visible transparent animationType="fade" onRequestClose={handleCancel}>
      <View style={[st.backdrop, { backgroundColor: palette.bg }]}>
        ...
        <View
          style={[st.frame, { width: FRAME_W, height: frameH }]}
