...
const services: Capability = {
  ...
  async run(): Promise<CapabilityResult> {
    ...
    const [rows, photos, specialties] = await Promise.all([
      getMyProviderServices(),
      getProviderPortfolio(profile.id),
      getProviderSpecialties(profile.id),
    ]);
    // Work gallery only. Venue/workspace shots aren't work a client browses —
    // they sit in Additional Information on the profile and never reach
    // Explore — so counting them here would both inflate the number and
    // silence the "no portfolio photos" gap for a provider whose gallery is
    // genuinely empty. getProviderPortfolio excludes them in SQL unless
    // includeVenue is asked for, which this count deliberately doesn't.
    const portfolio = photos.work;
    const active = rows.filter((s) => s.is_active);
