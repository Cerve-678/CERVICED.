...
-- Sends the client ONE "Address Now Available" notification when their booking
-- address becomes available, covering all three release paths:
--   1. on_confirmation  — this trigger
--   2. time-based       — hourly cron stamps address_released_at + notifies
--   3. manual           — app-side (releaseBookingAddress in databaseService).
--      ProviderBookingDetailScreen used to send its OWN separate "Address
--      Released" notification on top of this one — that was the duplicate
--      ("Address Now Available" + "Address Released" for the same event).
--      Fixed app-side: that screen no longer sends its own notification, so
--      this is the single source for the manual path too.
-- ════════════════════════════════════════════════════════════════════════════
...
-- ── 2. auto_release_address — stamp + notify (on_confirmation policy) ────────
...
-- ── 3. Cron function for time-based release policies — stamp + notify ────────
-- Runs hourly. Stamps address_released_at once the window opens and sends the
-- single "Address Now Available" notification for that booking.
