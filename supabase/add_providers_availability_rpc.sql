-- add_providers_availability_rpc.sql
-- Adds get_providers_availability(text[]) — batched, coarse near-term
-- availability status for a set of providers, keyed by slug, in ONE query.
--
-- Why: SearchScreen's result grid used to hardcode every card's "Slots
-- Available" badge to always be true (fake data, no filter behind it).
-- Real availability is computed per-provider from provider_availability +
-- windows + blocked dates + overrides + live bookings — running that per
-- card across a 200-provider search grid would be a textbook N+1. This RPC
-- rolls the whole set into one query instead, called once per search result
-- fetch from src/services/databaseService.ts's getProvidersAvailability().
--
-- Deployed live 2026-08-10 on project ztrfpfvvejzaysrelmfm via
-- mcp__claude_ai_Supabase__apply_migration (migration name:
-- get_providers_availability_batch). This file is the tracked record of
-- that live function — safe to re-run (CREATE OR REPLACE).
--
...
--
-- This mirrors the SAME day-level approximation
-- AvailabilityService.getAvailabilitySummary() uses for its own at-a-glance
-- strip dot ('open' when booked minutes < open minutes, 'full' otherwise) —
-- explicitly a coarse display signal, NEVER a booking gate. The real slot
-- simulation in AvailabilityService still owns anything that actually books
-- time.
--
-- has_gone_live + is_active gated (client-facing), matching the
-- provider-visibility-gating convention used elsewhere in this app.
-- SECURITY DEFINER so it can read schedule/booking rows behind RLS, but it
-- only ever emits a coarse status string — no private fields, no booking
-- details — for publicly listed providers.
...
--
--   SELECT * FROM public.get_providers_availability(
--     (SELECT array_agg(slug) FROM public.providers
--       WHERE has_gone_live AND is_active LIMIT 10)
--   );
-- ───────────────────────────────────────────────────────────
