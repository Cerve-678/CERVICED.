-- Category descriptions — shown to clients under the category tab on a
-- provider's profile (e.g. "Lip Fillers — subtle to fuller lip enhancement
-- for natural-looking volume").
--
-- Categories aren't their own row anywhere — a "category" is just the
-- category_name shared by a group of services — so the description is
-- stored redundantly on every service row in that category, the same way
-- category_name already is. The app writes the same value onto every
-- service in a category whenever it's added/edited.
--
