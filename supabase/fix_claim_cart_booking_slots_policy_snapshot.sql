-- CERVICED — claim_cart_booking_slots() must also snapshot policy acceptance.
--
-- Companion to fix_client_policy_acceptance.sql, which adds
-- bookings.policy_accepted_at / policy_snapshot. The fresh-INSERT path
-- (databaseService.createBooking) already writes these straight through, but
-- claim_cart_booking_slots() converts an existing on_hold row via UPDATE and
-- has its own explicit column list — same reason
-- fix_claim_cart_booking_slots_uses_real_address.sql had to patch this
-- function for provider_address_snapshot rather than relying on the insert
-- path alone. Without this, any booking made through a held slot (the common
-- case — CartScreen holds the slot before payment) would silently lose the
-- client's policy acceptance record even though the app sent it.
--
-- Rebuilds the function from the last-known-good definition in
-- fix_claim_cart_booking_slots_uses_real_address.sql, unchanged except for
-- the two new columns. Run after that file. Safe to re-run.
...
-- Existing held bookings are not touched. Future claims will now also
-- snapshot the client's provider-policy acceptance alongside the address fix
-- from fix_claim_cart_booking_slots_uses_real_address.sql.
