-- CERVICED — handle_booking_status_change() on_hold false-cancellation bug.
--
-- Root cause: the trigger's "real cancellation" branch fired whenever
-- NEW.status = 'cancelled' AND OLD.status was neither 'cancelled' nor
-- 'pending'. That condition doesn't exclude OLD.status = 'on_hold' — so
-- when a cart-checkout hold or waitlist hold expires and expire_cart_holds()
-- / expire_waitlist_holds() (crons, every 5/15 min) transition it
-- on_hold -> cancelled, this branch treats a routine, expected hold
-- expiry exactly like a genuine "your confirmed booking was cancelled"
-- event and fires a real booking_cancelled notification.
--
-- The v_actor IS NULL check in this branch means it only fires for
-- SYSTEM-initiated transitions (the crons run SECURITY DEFINER outside any
-- user session, so auth.uid() is NULL there) — a user's own explicit
-- release doesn't trigger it. That's exactly what made this look like a
-- spontaneous, unexplained cancellation notification for a booking the
-- recipient never actually had.
--
-- Confirmed live 2026-08-10: repeated "A client cancelled their Reserving…"
-- notifications (booking_id: null) on a real account — "Reserving…" is the
-- placeholder service_name_snapshot hold_cart_booking_slots() writes for an
-- unclaimed hold (see fix_hold_cart_booking_slots_missing_snapshots.sql),
-- the fingerprint of this exact bug.
--
-- Fix: add "AND OLD.status != 'on_hold'" to the branch condition. No other
-- logic changed. Rebuilt from the current live definition so nothing else
-- regresses.
--
-- Safe to re-run.
