...
--
--   1. provider_message ("Unread Message") — bare label title, no sense of
--      how long the message has been unread even though c.updated_at (the
--      staleness timestamp) was already selected and simply unused.
--   2. rebooking_nudge ("...misses you!") — static canned body with no
--      service name or actual elapsed time, even though the query already
--      knows exactly which service and exactly how long ago (that's what
--      HAVING MAX(b.booking_date) = ... computes to trigger the nudge).
--   3. birthday_greeting ("Happy Birthday!") — static canned body with no
--      customer name, even though `u` (the users row) was already joined.
--
-- Depends on process_birthday_greetings, process_rebooking_nudges, and
-- process_provider_unread_message_reminders already existing (see
-- notifications_cleanup_2026_08.sql for the first two's current bodies,
-- notification_recipient_role.sql for the third) — this file only changes
-- the SELECT list and the message/title strings, not any of the
-- eligibility/dedup logic (WHERE/HAVING/NOT EXISTS clauses are untouched).
-- ============================================================
...
--   SELECT prosrc FROM pg_proc WHERE proname = 'process_provider_unread_message_reminders';
--   SELECT prosrc FROM pg_proc WHERE proname = 'process_rebooking_nudges';
--   SELECT prosrc FROM pg_proc WHERE proname = 'process_birthday_greetings';
--     → expect the new title/message text in each
-- ============================================================
