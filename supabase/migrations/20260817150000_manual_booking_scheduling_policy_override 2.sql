-- Provider manual bookings: let the provider override scheduling-POLICY
-- warnings (booking window, minimum notice, provider-blocked date) instead
-- of hitting a hard, unexplained failure with no way to proceed.
--
-- BACKGROUND: 20260816120000_manual_booking_working_hours_bypass.sql already
-- built exactly this bypass pattern for the working-hours-fit check
-- (cerviced.bypass_working_hours), on the reasoning that a provider's own
-- manual "squeeze-in" booking is a deliberate policy override, not a
-- customer self-service booking that must respect every guardrail. This
-- migration extends the SAME reasoning to three more checks in
-- enforce_booking_bookability() that were left hard when that migration
-- shipped:
--   - 'Booking is outside this provider''s booking window'
--   - 'This appointment does not meet the provider''s minimum notice'
--   - 'Provider is unavailable on this date' (provider_blocked_dates only —
--     NOT the provider_availability_overrides is_closed check, which is
--     already covered by the existing working-hours bypass path)
--
-- DELIBERATELY LEFT HARD, NO BYPASS:
--   - 'Booking date cannot be in the past' — the date has already happened,
--     not a policy judgment call.
--   - Same-day booking for a time that has already passed, or is exactly
--     now — physically impossible to keep, not a notice-period preference.
--     Enforced by a NEW check below that fires unconditionally (bypass or
--     not) whenever booking_date = CURRENT_DATE.
--   - bookings_no_overlap (table EXCLUDE constraint) and the trigger's own
--     same-day overlap check ('That time is no longer available') — a slot
--     that's genuinely taken stays taken regardless of provider intent.
--
-- MECHANISM: a second transaction-local GUC, cerviced.bypass_scheduling_
-- policy, set only by provider_create_manual_booking() when its new
-- p_override_scheduling boolean is true. Client checkout, reschedule RPCs,
-- and waitlist invites never set it, so they are unaffected by construction.
--
-- App-side: AddBookingScreen now runs a client-side pre-check that mirrors
-- these three rules, and on the FIRST submit attempt sends
-- p_override_scheduling = false. If the RPC rejects with one of these three
-- specific messages, the screen shows the exact reason via a confirm dialog
-- ("Proceed anyway?") and retries once with p_override_scheduling = true.
-- The DB is still the final authority — the client-side pre-check is a UX
-- nicety that lets the confirm dialog appear before a round-trip, not a
-- security boundary.
--
-- Safe to re-run.
...
BEGIN
  ...
  -- Always enforced, bypass or not: a same-day booking for a time that has
  -- already elapsed (or is exactly now) cannot be kept by anyone, provider
  -- override or not.
  IF NEW.booking_date = CURRENT_DATE AND NEW.booking_time <= LOCALTIME THEN
    RAISE EXCEPTION 'That time has already passed today';
  END IF;

  -- Provider-initiated manual booking: skip the booking-window / minimum-
  -- notice / blocked-date POLICY checks below. Set exclusively by
  -- provider_create_manual_booking() via set_config('cerviced.bypass_
  -- scheduling_policy', 'on', true) when its p_override_scheduling argument
  -- is true; transaction-local, cannot leak to any other insert.
...
BEGIN
  ...
  -- Provider explicitly confirmed a scheduling-policy warning (booking
  -- window / minimum notice / blocked date) and wants to proceed anyway.
  -- Transaction-local; never set by any client-facing or automated path.
...
-- The old 7-arg overload (without p_override_scheduling) is superseded by
-- the 8-arg version above — drop it so PostgREST's schema cache has exactly
-- one candidate for the app's RPC call (an overload set would otherwise
-- force PostgREST to require the app send every named param to disambiguate).
