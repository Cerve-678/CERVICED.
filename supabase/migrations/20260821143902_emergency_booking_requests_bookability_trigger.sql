-- Emergency booking requests, part 2 of 5: the gate.
-- Introduced by 20260821143821_emergency_booking_requests.sql — see that
-- file for the full rationale.
-- Rewritten wholesale rather than patched: the four checks now each have a
-- three-way outcome (allowed / allowed-as-emergency / rejected) instead of
-- pass-or-raise, and the working-hours branch needs the envelope calculation
-- that neither previous version had.
...
DECLARE
  ...
  v_emergency BOOLEAN;
  -- Recurring weekly envelope, in minutes from midnight.
  v_env_open_mins INTEGER;
  v_env_close_mins INTEGER;
...
BEGIN
  SELECT booking_window_days, min_booking_notice_hrs,
         ...
         COALESCE(out_of_hours_extension_mins, 0)
  ...
  IF NOT v_bypass_hours THEN
    ...
    IF NOT COALESCE(v_fits_window, FALSE) THEN
      -- A shut day was already authorised by the blocked-date opt-in above;
      -- anything else here is genuinely "outside working hours" and needs
      -- that opt-in instead.
      IF NOT v_emergency OR NOT (v_allow_hours OR (v_day_is_shut AND v_allow_blocked)) THEN
        RAISE EXCEPTION 'This appointment is outside the provider''s working hours';
      END IF;

      -- Bound it. The envelope is the earliest start and latest end across
      -- the provider's whole recurring week, widened by their extension
      -- setting — deliberately NOT "any time on a 24h clock". Minute
      -- arithmetic rather than TIME +/- INTERVAL, which wraps around
      -- midnight (09:00 - 2h = 07:00, but 01:00 - 2h = 23:00, silently
      -- inverting the comparison).
      SELECT (EXTRACT(EPOCH FROM MIN(start_time)) / 60)::int,
             (EXTRACT(EPOCH FROM MAX(end_time)) / 60)::int
        INTO v_env_open_mins, v_env_close_mins
        FROM public.provider_availability_windows
       WHERE provider_id = NEW.provider_id;

      IF v_env_open_mins IS NULL THEN
        SELECT (EXTRACT(EPOCH FROM MIN(open_time)) / 60)::int,
               (EXTRACT(EPOCH FROM MAX(close_time)) / 60)::int
          INTO v_env_open_mins, v_env_close_mins
          FROM public.provider_availability
         WHERE provider_id = NEW.provider_id AND NOT COALESCE(is_closed, TRUE);
      END IF;

      -- No recurring schedule at all means no envelope to widen. Rejected
      -- rather than treated as "anything goes".
      IF v_env_open_mins IS NULL THEN
        RAISE EXCEPTION 'This appointment is outside the provider''s working hours';
      END IF;

      v_start_mins := (EXTRACT(EPOCH FROM NEW.booking_time) / 60)::int;
      v_end_mins   := (EXTRACT(EPOCH FROM NEW.end_time) / 60)::int;

      IF v_start_mins < GREATEST(0, v_env_open_mins - v_extension_mins)
         OR v_end_mins > LEAST(1440, v_env_close_mins + v_extension_mins) THEN
        RAISE EXCEPTION 'This appointment is too far outside the provider''s working hours';
      END IF;
    END IF;
  END IF;
