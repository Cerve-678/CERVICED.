...
-- BEFORE they book. Safe to re-run.
--
-- `provider_form_library` is owner-only ("provider_form_library_all" restricts
-- every operation to the owning provider), so a client could not read a byte
-- of it. The booking sheet needs the terms as a read-only pop-up at the point
-- of booking. That must NOT be done by loosening the table's RLS: the library
-- also holds medical-history and patch-test forms whose questions are nobody
-- else's business. Instead, one narrow SECURITY DEFINER function returns only
-- the terms form's title and body text, and only for a provider who is live.
