...
--
--   * transactions.booking_id is NOT NULL REFERENCES bookings(id) with no
--     ON DELETE rule. If money is attached the row is NOT a phantom --
--     payment succeeded and the claim failed -- and it must survive for
--     someone to look at. It is left alone.
--   * reviews has the same shape. A hold cannot have one, but the guard
--     costs nothing and means this cannot be the statement that loses one.
--
