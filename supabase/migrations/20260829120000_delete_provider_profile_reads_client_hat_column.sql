-- ============================================================================
-- NOT APPLIED YET. Written 2026-08-29; the session that wrote it could not
-- reach the live project (apply_migration was refused), so this has never run.
-- Before applying: 20260829120000 is a placeholder above the then-frontier
-- (max(version) = 20260828192643). apply_migration stamps its OWN version, so
-- rename this file to whatever version is actually recorded, then release the
-- lock in supabase/MIGRATION_OWNER.md, which is held for this file.
-- Checked against live data when written: every live provider row classifies
-- identically under the old and the new read, so this corrects the rule
-- without reclassifying any existing account.
-- ============================================================================
