...
-- here.
--
-- Applied live 2026-09-03, recorded as this version (renamed from its
-- authored 20260901180000). Verified functionally in a rolled-back
-- transaction against a real provider (11->9 in an earlier exploratory
-- count was a join-fanout artifact of that query, not data loss — the
-- rollback left the provider's real 9 services / 5 add-ons unchanged):
...
-- untouched add-on kept its id; a service omitted from the payload was
-- deleted along with its add-on (cascade); a brand-new service was
-- inserted; and an incoming id belonging to a DIFFERENT provider's service
-- was correctly rejected (inserted as a new row instead of overwriting),
-- leaving the other provider's row completely untouched. Grants preserved
-- exactly (CREATE OR REPLACE on the same signature, not a DROP+CREATE
-- pair): anon -> false, authenticated -> true.
