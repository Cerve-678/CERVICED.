-- Suitable hair types — shown to clients on a HAIR-category service so they
-- can tell at a glance whether a service fits their hair type before booking
-- (e.g. a curly-specialist service vs. a straight/fine-hair blowout), using
-- the same HAIR_TYPES vocabulary already used for the client's own hair type
-- (BeautyProfileScreen.tsx, SignUpStep4Screen.tsx): Straight, Wavy, Curly,
-- Coily, 4A, 4B, 4C. Provider-set, multi-select, optional (a service with no
-- values set is treated as suiting all hair types).
--
-- Run in the Supabase SQL editor. Safe to re-run.

ALTER TABLE public.services
  ADD COLUMN IF NOT EXISTS hair_types_suitable TEXT[];

-- replace_provider_services() needs to carry the new field through its
-- delete + reinsert — CREATE OR REPLACE so this is safe to re-run after
-- supabase/replace_provider_services.sql.
