...
export default function NotificationsScreen({ navigation }: HomeScreenProps<'Notifications'>) {
  ...
  const handleNotificationAction = useCallback((notification: Notification) => {
    console.log('🔔 [NotificationsScreen] handleNotificationAction called');
    ...
    if (notification.type === 'booking_confirmed' ||
      ...
      // Close the Notifications screen (modal), then navigate to Bookings
      setTimeout(() => {
        // Go back to close the modal
        navigation.goBack();

        // Then navigate to Bookings after modal closes
        setTimeout(() => {
          try {
            if (notification.bookingId) {
              ...
              navigation.navigate('Bookings', undefined);
            }
            console.log('✅ Navigation to Bookings executed successfully');
          } catch (error) {
      ...
      console.log('🚀 Navigating to Explore tab');
      setShowMessagePopup(false);
      setSelectedNotification(null);
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          try {
            ...
            console.log('✅ Navigation to Explore executed');
          } catch (error) {
...
const styles = StyleSheet.create({
  ...
  // Notifications List
  notificationsContent: { paddingHorizontal: 20, paddingBottom: 120 },
  notificationItem: { marginBottom: 16 },
  notificationBlur: {
    borderRadius: 20,
    borderWidth: 1.5,
    borderColor: 'rgba(255,255,255,0.4)',
  ...
  notificationLeft: { alignItems: 'center', gap: 8 },
  iconContainer: { 
    width: 40, 
    height: 40, 
    justifyContent: 'center', 
