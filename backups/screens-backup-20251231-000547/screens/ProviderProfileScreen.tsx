...
const AddOnsModal: React.FC<AddOnsModalProps> = React.memo(
  ({ isVisible, onClose, service, onAddToCart, adaptiveAccentColor }) => {
    ...
    // Available add-ons for any service
...
const ReviewsModal: React.FC<ReviewsModalProps> = React.memo(
  ({ isVisible, onClose, reviews, providerName, adaptiveAccentColor }) => {
    ...
    return (
      ...
      >
        ...
        >
          <LinearGradient
            colors={['rgba(255,255,255,0.15)', 'rgba(240,240,255,0.10)']} // Much more transparent
...
const ProviderProfileScreen: React.FC<ProviderProfileScreenProps> = ({ navigation, route }) => {
  ...
  Vibration.vibrate(50); // ADD VIBRATION HERE
 ...
 const handleBookmarkToggle = useCallback(async () => {
  ...
  Vibration.vibrate(50); // Vibration for bookmark
  setIsBookmarkLoading(true);
  
  try {
    if (providerIsBookmarked) {
      await removeBookmark(providerId);
      console.log('❌ Bookmark removed:', providerId);
      ...
      console.log('✅ Provider bookmarked:', providerId);
      setNotificationMessageType('bookmark'); // BOOKMARK MESSAGE TYPE
    ...
    try {
      ...
      if (supported) {
        ...
        Alert.alert(
          ...
          [
            { text: 'Copy Email', onPress: () => console.log('Email copied:', email) },
            { text: 'OK' },
          ]
        );
      }
  ...
  const handleQuickBook = useCallback(
    (service: ServiceData) => {
      console.log('Quick Book - Redirecting to checkout:', service.name);
  ...
  const handleAddToCartWithAddOns = useCallback(
    (service: ServiceData, selectedAddOns: Array<{ id: number; name: string; price: number }>) => {
      console.log('Book with Add-ons - Adding to cart:', service.name, selectedAddOns);
